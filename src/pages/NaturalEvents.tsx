...
import { IonButtons, IonContent, IonHeader, IonMenuButton, IonPage, IonCard, IonTitle, IonToolbar, IonCardHeader, IonCardSubtitle, IonCardTitle, IonCardContent, IonText, IonButton, IonGrid, IonCol, IonIcon, IonList, IonItem } from '@ionic/react';
    ...
    useEffect(() => {
        ...
        axios.get(`${rootUrl}/events`, config)
            .then(resp => {
                ...
                // foreach( data.events, function( key, event ) {
                //     $( "#eventList" ).append(
                //         "<dt class='event'>" +
...
//   }
    ...
    // Beware, sloppy typing here
    const makeList = (datas: dataItem[]) => datas.map((data: dataItem) => (
        <IonItem key={data.id}>{data.title}</IonItem>
    ));
